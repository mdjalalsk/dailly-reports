			...
			if ( ! empty( $desired_slug ) ) {
				...
				if ( ! empty( $subdir_name ) && $subdir_name !== $desired_slug ) {
					...
					if ( true === $GLOBALS['wp_filesystem']->move( $from_path, $to_path ) ) {
						...
						return new WP_Error( 'rename_failed', esc_html__('The remote plugin package does not contain a folder with the desired slug and renaming did not work.', 'tgmpa') . ' class-tgm-plugin-activation.php' . esc_html__( 'Please contact the plugin provider and ask them to package their plugin according to the WordPress guidelines.', 'tgmpa' ), array( 'found' => $subdir_name, 'expected' => $desired_slug ) );
					}
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					foreach ( $message as $type => $plugin_group ) {
						...
						$imploded       = empty( $linked_plugins ) ? $last_plugin : ( implode(', ', $linked_plugins) . ' class-tgm-plugin-activation.php' . esc_html_x( 'and', 'plugin A *and* plugin B', 'tgmpa' ) . ' ' . $last_plugin );
		...
		public function process_bulk_actions() {
			...
			if ( 'tgmpa-bulk-activate' === $this->current_action() ) {
				...
				if ( is_wp_error( $activate ) ) {
					...
					$imploded     = empty( $plugin_names ) ? $last_plugin : ( implode(', ', $plugin_names) . ' class-tgm-plugin-activation.php' . esc_html_x( 'and', 'plugin A *and* plugin B', 'tgmpa' ) . ' ' . $last_plugin );
